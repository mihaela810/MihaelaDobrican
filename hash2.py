...
def search_cnp_hash(cnp):
    ...
    if index in hash_table:
        for entry in hash_table[index]:
            iterations += 1
            if entry[2] == cnp:
                list_i.append(iterations)
                return entry, iterations, list_i
    return None, iterations, list_i
...
for cnp in sample_cnps:
    result_hash, iterations_hash, list_i = search_cnp_hash(cnp)
 ...
 #   print(f"Media este {medie}")
    list_i_tot.extend(list_i)
    #if list_i_tot:
    avg_iterations = sum(list_i_tot) // len(list_i_tot)
print(f"Media iterațiilor: {avg_iterations}")
    #else:
        #print("Nu s-au efectuat căutări valide pentru calculul mediei.")
...
total_county_generated = sum(county_count.values())
county_percentages = {k: (v / total_county_generated) * 100 for k, v in county_count.items()}

procente_reale = {
    'Alba': 2.1, 'Arad': 2.0, 'Argeș': 2.3, 'Bacău': 3.2, 'Bihor': 2.5, 'Bistrița-Năsăud': 1.8, 'Botoșani': 2.2, 'Brăila': 2.0,
    'Brașov': 2.8, 'București': 10.5, 'Buzău': 2.4, 'Călărași': 1.2, 'Caraș-Severin': 1.4, 'Cluj': 4.0, 'Constanța': 2.5,
    'Covasna': 1.5, 'Dâmbovița': 2.6, 'Dolj': 3.0, 'Galați': 2.1, 'Giurgiu': 1.3, 'Gorj': 1.7, 'Harghita': 1.6, 'Hunedoara': 1.2,
    'Ialomița': 1.5, 'Iași': 4.5, 'Ilfov': 1.4, 'Maramureș': 2.0, 'Mehedinți': 2.1, 'Mureș': 2.5, 'Neamț': 1.9, 'Olt': 3.2,
    'Prahova': 2.4, 'Sălaj': 1.8, 'Satu Mare': 1.6, 'Sibiu': 3.0, 'Suceava': 3.5, 'Teleorman': 1.1, 'Timiș': 2.0, 'Tulcea': 2.6,
    'Vâlcea': 1.8, 'Vaslui': 1.6, 'Vrancea': 1.8
}
...
def draw_histogram_comparison(canvas, generated_data, real_data, x, y, width, height, title):
    max_value = max(max(generated_data.values(), default=1), max(real_data.values(), default=1))
    bar_width = width / (2 * len(generated_data))

    canvas.create_text(x + width / 2, y + height + 70, text=title, font=("Arial", 12, "bold"))

    ##sorted_items = sorted(generated_data.items(), key=lambda item: counties[item[0]])

    for i, county_code in enumerate(generated_data.keys()):
        gen_value = generated_data[county_code]
        real_value = real_data.get(county_code, 0)
        gen_height = (gen_value / max_value) * height
        real_height = (real_value / max_value) * height

        canvas.create_rectangle(
            x + i * 2 * bar_width, y + height - gen_height,
            x + (i * 2 + 1) * bar_width, y + height,
            fill="blue", outline="black"
        )

        canvas.create_text(
            x + i * 2 * bar_width + bar_width / 2, y + height - gen_height - 20,
            text=f"{gen_value:.1f}%", font=("Arial", 8), fill="blue", angle=90
        )

        canvas.create_rectangle(
            x + (i * 2 + 1) * bar_width, y + height - real_height,
            x + (i * 2 + 2) * bar_width, y + height,
            fill="red", outline="black"
        )

        canvas.create_text(
            x + (i * 2 + 1) * bar_width + bar_width / 2, y + height - real_height - 20,
            text=f"{real_value:.1f}%", font=("Arial", 8), fill="red", angle=90
        )

        canvas.create_text(
            x + i * 2 * bar_width + bar_width, y + height + 30,
            text=county_code, font=("Arial", 8), angle=90
        )
...
x_start, y_start = 15, 210
width, height = 1200, 300
draw_histogram_comparison(canvas, county_percentages, procente_reale, x_start, y_start, width, height,
                          "Distribuție pe județe (Comparativ)")
